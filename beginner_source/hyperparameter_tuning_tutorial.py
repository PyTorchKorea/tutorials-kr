...
# 이 방법을 통해 서로 다른 시도(trials)간 데이터 디렉토리(directory)를 공유할 수 있습니다.
...
# 완전 연결 계층(fully connected layers)에서 계층(layer)의 크기가 해당됩니다.
...
# 짐작한 것 처럼 ``config`` 파라미터는 훈련하고자 하는 하이퍼파라미터들을 전달받게됩니다.
