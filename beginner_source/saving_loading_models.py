...
# 를 사용하여 사전을 불러옵니다. 이후로는 저장된 항목들을 원하는 대로 사전에
# 질의하여 쉽게 접근할 수 있습니다.
#
# 추론을 실행하기 전에는 반드시 ``model.eval()`` 을 호출하여 드롭아웃 및 배치
# 정규화를 평가 모드로 설정하여야 합니다. 이것을 하지 않으면 추론 결과가 일관성
...
######################################################################
# 여러 개(multiple)의 모델을 하나의 파일에 저장하기
# -------------------------------------------------------
#
# 저장하기:
# ^^^^^^^^^^^^^^
...
#    modelB.train()
#
# GAN, Seq2Seq 또는 앙상블 모델과 같이 여러 개의 ``torch.nn.Modules`` 로
...
# GPU 장치에 불러옵니다. 다음으로 ``model.to(torch.device('cuda'))`` 을 호출하여
