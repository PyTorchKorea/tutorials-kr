...
# -------
#
# 본 튜토리얼에서는 예제를 통해 DCGAN을 알아보겠습니다. 실제 유명인들의 사진들로 적대적 생성 신경망(GAN)을 학습하여
# 새로운 유명인의 사진을 만들어보겠습니다.
# 사용할 대부분의 코드는 `pytorch/examples <https://github.com/pytorch/examples>`__ 의 DCGAN 구현에서 가져왔으며,
# 본 문서는 구현에 대한 설명과 함께 이 모델이 작동하는 이유에 대해 설명을 해줄 것입니다.
# 입문자라면 실제로 모델에 무슨 일이 일어나고 있는지 이해하는 데 다소 시간이 걸릴 수 있으나,
# GAN에 대한 사전 지식이 필수는 아니니 걱정하지 않으셔도 됩니다.
# 또한 GPU 1~2개를 사용하는 것이 시간 절약에 도움이 될 겁니다. 그럼 처음부터 천천히 시작해 봅시다!
#
...
# 일 때이며, 이때 구분자는 입력이 진짜인지 가짜인지를 무작위로 추측하게
...
# 합성곱 신경망(convolution)과 전치 합성곱 신경망(convolutional-transpose)을 사용했다는 것이 차이점입니다.
# Radford 등이 저술한 `Unsupervised Representation Learning With
# Deep Convolutional Generative Adversarial
# Networks <https://arxiv.org/pdf/1511.06434.pdf>`__ 논문에서 처음 모델이 소개되었고, 지금은 대부분의 GAN 모델이
# DCGAN을 기반으로 만들어지는 중입니다. 이전 GAN과 모델의 구조가 실제로 어떻게 다른지 확인해보면, 먼저 구분자에서는
...
# 활성함수가 사용되었습니다. 클래식한 GAN과 마찬가지로, 구분자의 입력 데이터는 3x64x64 크기의 이미지이고,
# 출력값은 입력 데이터가 실제 데이터일 0~1 사이의 확률값입니다.
# 다음으로, 생성자는
...
# (번역 주. 전치 합성곱 신경망은 합성곱 신경망의 반대 개념으로 이해하면 쉽습니다. 입력된 작은 CHW 데이터를 가중치들을 이용해 더 큰 CHW로 업샘플링해주는 계층입니다.)
# 논문에서는 각종 최적화 방법이나 손실 함수의 계산, 모델의 가중치 초기화 방법 등에 관한 추가적인 정보들도 적어두었는데,
...
#    설정합니다.
# -  ``beta1`` - Adam 옵티마이저에서 사용할 beta1 하이퍼파라미터값입니다.
...
# 모든 합성곱 계층, 전치 합성곱 계층, 배치 정규화 계층의 가중치들을 위에서 말한 조건대로
# 다시 초기화합니다. 이 함수는 모델이 만들어지자마자 바로 적용합니다.
...
# 변환하도록 설계되었습니다. 우리에게 데이터라 함은 이미지이기 때문에,
# :math:`z` 를 데이터 공간으로 변환한다는 뜻은, 학습이미지와 같은 사이즈를 가진
...
# 데이터를 가공하고, 마지막 출력에서 Sigmoid 함수를 이용하여
# 0~1 사이의 확률값으로 조정합니다. 이 아키텍처는 필요한 경우 더 다양한 레이어를 쌓을 수 있지만,
# 배치 정규화와 LeakyReLU, 특히 보폭이 있는 (strided) 합성곱 계층을
# 사용하는 것에는 이유가 있습니다.
# DCGAN 논문에서는 보폭이 있는 합성곱 계층을 사용하는 것이 신경망 내에서 스스로의
# 풀링(Pooling) 함수를 학습하기 때문에, 데이터를 처리하는 과정에서 직접적으로 풀링 계층( MaxPooling or AvgPooling)을
# 사용하는 것보다 더 유리하다고 합니다. 또한 배치 정규화와 LeakyReLU 함수는 학습 과정에서
# :math:`G` 와 :math:`D` 가 더 효과적인 경사도(gradient)를 얻을 수 있도록 돕습니다.
#
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# :math:`D` 와 :math:`G` 의 설정을 끝냈으니, 이제 손실 함수와 옵티마이저를 정하여
# 학습을 구체화할 시간입니다. 손실 함수로는 Binary Cross Entropy loss
# (`BCELoss <https://pytorch.org/docs/stable/generated/torch.nn.BCELoss.html#torch.nn.BCELoss>`__)
# 를 사용할 것입니다. 해당 함수는 아래의 식으로 파이토치에 구현되어 있습니다:
#
# .. math:: \ell(x, y) = L = \{l_1,\dots,l_N\}^\top, \quad l_n = - \left[ y_n \cdot \log x_n + (1 - y_n) \cdot \log (1 - x_n) \right]
#
# 이때, 위의 함수가 로그함수 요소를 정의한 방식을 주의 깊게 봐주세요 (예. :math:`log(D(x))` 와
# :math:`log(1-D(G(z)))`). 우린 :math:`y` 을 조정하여, BCE 함수에서
# 사용할 요소를 고를 수 있습니다.
...
# 두겠습니다. 각 라벨의 값을 정한 건 GAN 논문에서 사용된 값들로, GAN을 구성할 때의 관례라 할
# 수 있습니다. 방금 정한 라벨값들은 추후에 손실값을 계산하는 과정에서 사용될 것입니다.
...
# 프로그램이 끝날 때까지 고정된 잠재공간 벡터를 생성하겠습니다 (예. fixed_noise).
# 이 벡터들 역시 가우시안 분포에서 추출합니다. 학습 과정을 반복하면서  :math:`G` 에 주기적으로 같은 잠재공간 벡터를
# 입력하면, 그 출력값을 기반으로 생성자의 상태를 확인할 수 있습니다.
#
...
# 실제 모델을 학습하는 방법을 알아보겠습니다. 주의를 기울일 것은, GAN을 학습하는 것은
# 관례적인 기술들의 집합이기 때문에, 잘못된 하이퍼파라미터의 설정은
# 모델의 학습을 망가뜨릴 수 있습니다. 무엇이 잘못되었는지 알아내는 것조차도 힘들죠.
# 그러한 이유로, 본 튜토리얼에서는 `Goodfellow’s paper <https://papers.nips.cc/paper/5423-generative-adversarial-nets.pdf>`__
# 에서 서술된 Algorithm 1을 기반으로, `ganhacks <https://github.com/soumith/ganhacks>`__ 에서 사용된 몇 가지 괜찮은 테크닉들을
# 더할 것입니다. 앞서 몇 번 설명했지만, 우리의 의도는 “진짜 혹은 가짜 이미지를 구성”하고,
# :math:`log(D(G(z)))` 를 최대화하는 G의 목적함수를 최적화하는 것입니다. 학습 과정은 크게 두 가지로 나눕니다.
# Part 1은 구분자를, Part 2는 생성자를 업데이트하는 과정입니다.
#
# **Part 1 - 구분자의 학습**
#
# 구분자의 목적은 주어진 입력값이 진짜인지 가짜인지 판별하는 것임을 상기합시다.
# Goodfellow의 말을 빌리자면, 구분자는 “변화도(gradient)를 상승(ascending)하며 훈련”하게 됩니다.
# 실전적으로 얘기하면, :math:`log(D(x)) + log(1-D(G(z)))` 를 최대화하는 것과 같습니다.
...
# 역전파 과정에서의 변화도들을 계산합니다. 여기까지가 첫 번째 스텝입니다. 두 번째 스텝에서는, 오로지 가짜 데이터들로만
...
# 변화도까지 구했으니, 이제 옵티마이저를 사용해야겠죠. 파이토치의 함수를 호출해주면 알아서 변화도가 적용될 것입니다.
#
# **Part 2 - 생성자의 학습**
#
# 오리지널 GAN 논문에 명시되어 있듯, 생성자는 :math:`log(1-D(G(z)))` 을 최소화하는 방향으로 학습합니다.
# 하지만 이 방식은 충분한 변화도를 제공하지 못함을 Goodfellow가 보여줬습니다. 특히 학습 초기에는 더욱 문제를 일으키죠.
...
# 그러면 구해진 손실값으로 변화도를 구하고, 최종적으로는 옵티마이저를 이용해 G의 가중치들을 업데이트해주면 됩니다.
# 언뜻 볼 때는, 생성자가 만들어낸 *가짜* 이미지에 *진짜* 라벨을 사용하는것이 직관적으로 위배가 될 것처럼 보이지만, 이렇게 라벨을
...
# -  **D(G(z))** - 가짜 데이터들에 대한 구분자의 출력값입니다. 처음에는 0에 가까운 값이다가,
...
# ------
#
# 결과를 알아봅시다. 이 섹션에서는 총 세 가지를 확인할겁니다.
# 첫 번째는 G와 D의 손실값들이 어떻게 변했는가, 두 번째는 매 에폭마다
...
# **G의 학습 과정 시각화**
#
# 매 에폭마다 fixed_noise를 이용해 생성자가 만들어낸 이미지를 저장한 것을 기억할 겁니다.
# 저장한 이미지들을 애니메이션 형식으로 확인해 봅시다. play 버튼을 누르면 애니매이션이 실행됩니다
#
...
# -  다른 데이터셋을 이용해 훈련시켜보거나, 이미지의 사이즈를 다르게 해보거나, 아키텍처의 구성을 바꿔볼 수도 있습니다
