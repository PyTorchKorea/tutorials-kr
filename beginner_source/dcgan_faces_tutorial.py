...
#
# 이때, 위의 함수가 로그함수 요소를 정의한 방식을 주의깊게 봐주세요 (i.e. :math:`log(D(x))` 와
# :math:`log(1-D(G(z)))`). 우린 :math:`y` 을 조정을 조정하여, BCE 함수에서
# 사용할 요소를 고를 수 있습니다.
# 이 부분은 이후에 서술할 학습 섹션에서 다루겠지만, 어떻게 :math:`y` 를 이용하여
# 우리가 원하는 요소들만 골라낼 수 있는지 이해하는 것이 먼저입니다 (i.e. GT labels).
#
# 좋아요, 다음으로 넘어가겠습니다. 참 라벨 (혹은 정답)은 1로 두고, 거짓 라벨 (혹은 오답)은 0으로
...
# 프로그램이 끝날때까지 고정된 잠재공간 벡터를 생성하겠습니다 (i.e. fixed_noise).
...
# 일종의 관례적인 기술들을 집합한 것이기 때문에, 잘못된 하이퍼파라미터의 설정은
...
# Goodfellow의 말을 빌리자면, 구분자는 “그래디언트를 상승(ascending)시키며 훈련”하게 됩니다.
...
# 역전파 과정에서의 그래디언트들을 계산합니다. 여기까지가 첫번째 스텝입니다. 두번째 스텝에서는, 오로지 가짜 데이터들로만
# 이루어진 배치를 만들어 :math:`D` 에 통과시키고, 그 출력값으로 (:math:`log(1-D(G(z)))`) 의 손실값을 계산해
# 역전파 그래디언트를 구하면 됩니다. 이때 두가지 스텝에서 나오는 그래디언트들은 *축적(accumulate)* 시켜야 합니다.
# 그래디언트까지 구했으니, 이제 옵티마이저를 사용해야겠죠. 파이토치의 함수를 호출해주면 알아서 그래디언트가 적용될겁니다.
#
# **Part 2 - 생성자의 학습**
#
# 오리지널 GAN 논문에 명시되어 있듯, 생성자는 :math:`log(1-D(G(z)))` 을 최소화시키는 방향으로 학습합니다.
# 하지만 이 방식은 충분한 그래디언트를 제공하지 못함을 Goodfellow가 보여줬습니다. 특히 학습초기에는 더욱 문제를 일으키죠.
...
# 그러면 구해진 손실값으로 그래디언트를 구하고, 최종적으로는 옵티마이저를 이용해 G의 가중치들을 업데이트시켜주면 됩니다.
...
#
#
# **Note:** 이후의 과정은 epoch의 수와 데이터의 수에 때라 시간이 좀 걸릴 수 있습니다
#
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        # 역전파를 통해 그래디언트를 계산합니다. 이때 앞서 구한 그래디언트에 더합니다(accumulate)
        errD_fake.backward()
...
# 첫번째는 G와 D의 손실값들이 어떻게 변했는가, 두번째는 매 에포크마다
...
# **G의 학습 과정 시각화**
#
# 매 에포크마다 fixed_noise를 이용해 생성자가 만들어낸 이미지를 저장한 것을 기억할겁니다.
...
# 무엇을 더 시도해볼 수 있을까요?
#
# -  학습시간을 더욱 늘려 G의 성능을 향상시킬 수 있습니다
# -  다른 데이터셋을 이용해 훈련시켜보거나, 이미지의 사이즈를 다르게 해보거나, 아키텍쳐의 구성을 바꿔볼 수 있습니다
