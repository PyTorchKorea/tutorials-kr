...
단어 임베딩이(word embedding)란 코퍼스(corpus) 내 각 단어에 일대일로 대응하는 밀집된 실수 벡터(dense vector)의 집합, 혹은 이 벡터를
...
단어들은 서로 의미적 연관성을 가진다는 것입니다. 언어학적으로는 '분산 의미 가설' 혹은 `distributional
...
이 인덱스를 룩업테이블(look-up table)에서 사용할 것입니다. 즉, :math:`|V| \times D` 크기의 행렬에
