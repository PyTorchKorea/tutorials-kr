...
단어 임베딩이란 코퍼스 내 각 단어에 일대일로 대응하는 농밀한 실수 벡터의 집합,
혹은 이 벡터를 구하는 행위를 가리킵니다. 먼저, 단어를 주로 피처로 사용하는
자연어 처리 분야에서는 단어를 컴퓨터 친화적인 형태로 바꾸어 주는 작업이 필수적입니다.
컴퓨터가 단어를 바로 이해하기는 상당히 어렵기 때문이죠.
...
원핫 벡터는 모든 단어를 독립적인 개체로 가정하는 것을 볼 수 있습니다. 즉, 공간에서 봤을 때
...
단어들은 서로 의미적 연관성을 가진다는 것입니다. 언어학적으로는 '분산의미가설' 혹은 `distributional
...
단어 벡터도 함께 업데이트 되도록 하면 될 것입니다. 이렇게 우리 신경망 모델은 적어도 이론상으로는
...
이 인덱스를 룩업테이블에서 사용할 것입니다. 즉, :math:`|V| \times D` 크기의 행렬에
...
# 원래는 입력을 제대로 토크나이즈 해야하지만 이번엔 간소화하겠습니다.
...
# 단어를 예측하려는 모델입니다. 언어 모델링과는 다르게 CBOW는 순차적이지 않고
# 확률적이지도 않습니다. 주로 CBOW는 복잡한 모델의 초기 입력값으로 쓰일
# 단어 임베딩을 빠르게 학습하는 데에 주로 쓰입니다. 보통 이것을 *프리트레인 된
# 임베딩* 이라고 부르죠. 몇 퍼센트 정도의 성능 향상을 기대할 수 있는 기법입니다.
#
# CBOW모델은 다음과 같습니다. 목표 단어 :math:`w_i`와 그 양쪽에 :math:`N`개의
# 문맥 단어 :math:`w_{i-1}, \dots, w_{i-N}` 와 :math:`w_{i+1}, \dots, w_{i+N}`
# 가 주어졌을 때, (문맥 단어 모두를 총칭해 :math:`C`라고 합시다.)
#
...
# 중복된 단어를 제거하기 위해 raw_text를 set으로 바꿔줍니다.
vocab = set(raw_text)
