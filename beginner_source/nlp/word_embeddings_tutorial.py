...
구하는 행위를 가리킵니다. 주로 단어를 피처로 사용하는 자연어 처리 분야에서는 단어를 컴퓨터 친화적인
...
먼저, 상기한 ASCII코드 대신 원핫 인코딩을 사용해보는 것은 어떨까요? 원핫 인코딩이란
...
이를 이용해 단어 벡터를 모델 파라미터로 설정하고 모델 학습시에 단어 벡터도 함께 업데이트 하면
