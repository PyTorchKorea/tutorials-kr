...
시퀀스 모델은 NLP의 핵심입니다 : 이는 입력 간에 일종의 시간적 종속성이 존재하는 모델입니다. 
시퀀스 모델의 고전적인 예는 부분 음성 태깅을 위한 히든 마르코프 모델입니다. 
또 다른 예는 조건부 랜덤 필드입니다. 

순환 신경망은 일종의 상태를 유지하는 네트워크입니다. 
예를 들면, 출력은 다음 입력의 일부로 사용될 수 있어
...
# 비터비(Viterbi)나 정방향-역방향(Forward-Backward) 같은 것들은 사용하지 않을 것입니다. 
...
for sent, tags in training_data:
    for word in sent:
        if word not in word_to_ix:  # word는 아직 인덱스가 할당되지 않았습니다
            word_to_ix[word] = len(word_to_ix)  # 각 단어에 고유한 인덱스 할당
print(word_to_ix)
tag_to_ix = {"DET": 0, "NN": 1, "V": 2}  # 각 태그에 고유한 인덱스 할당
