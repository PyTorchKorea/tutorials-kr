...
Pytorch는 *동적* 신경망 툴킷입니다. 다른 동적인 신경망 툴킷 중에는
`Dynet <https://github.com/clab/dynet>`_ 이 있습니다.(이 툴킷을
쓰는 법이 Pytorch와 비슷하여 예시를 들었습니다. Dynet의 예제를 보면
Pytorch로 구현할 때도 도움이 될 것입니디.) 반대로 *정적*인 툴킷들로
...
모델은 좋은 개체명 인식 모델에 꼭 필요합니다. 여러분이 CRF를 잘 알고
있다고 가정하겠습니다. 무섭게 들릴 수도 있겠지만, LSTM이 특징을
제공하는 모든 모델들은 CRF 입니다. 하지만 이 모델은 더 발전된 모델이며,
...
정의합니다. :math:`i`번째 단어에 대한 배출 포텐셜은 Mi-LSTM의
