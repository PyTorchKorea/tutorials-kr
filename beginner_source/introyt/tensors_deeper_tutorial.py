...
# ~~~~~~~~~~~~~
# 
# 두개 혹은 그 이상의 tensor에 대한 연산을 수행할 때, tensor들은 같은 shape를 필요로 합니다
...
# - Pytorch에서는 첫번째 인수로 tensor shape라는 값을 의미하는 라벨이 없는 정수 인자를 여러개를 받습니다 -
...
##########################################################################
# 여기서 주목해야할 점은 이전 코드 cell에 있는 모든 tensor는 동일한 shape를 가져야 한다는 것 입니다.
...
# 동일한 shape의 학습 가중치 tesnor를 반환하는 것입니다.
# 
...
# 만약 ``out`` tensor가 알맞는 shape와 ``dtype`` 을 가지고 있다면,
...
# 아래에 있는 몇가지 흔한 경우와 함께 tensor의 shape를 다루는 방법에 대해 알아볼 것 입니다.
# 
# 차원의 개수 변경하기
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 
...
# 무작위 3 x 1 shape의 tensor를 ``a`` 의 원소 개수가 3인 모든 열에 곱하는 것이였습니다.
# 
