...
이 글을 읽고 있다면, 머신 러닝 모델이 얼마나 효과적인지 진가를 알 수 있을 것입니다.
머신 러닝 연구는 ML(Machine Learming) 모델을 더욱 빠르고 정확하며 효율적이 되는 방향으로 진행 되고 있습니다.
그러나, 모델을 설계하고 훈련하는 데 종종 간과되는 측면은 모델을 속이려하는 적을 대면했을 때의 보안과 견고성입니다.

이 튜토리얼은 ML 모델들의 보안 취약점에 대한 인식을 높이고, 요즘 이슈가 되는 적대적 머신 러닝 주제에 대한 통찰력을 제공합니다.
이미지에 감지 할 수 없는 작은 변화(perturbation)를 추가하면 모델 성능이 크게 달라질 수 있다는 사실에 놀랄 수 있습니다.
...
# ------------
# 
# 상황에 따라 다양한 범주의 적대적 공격이 있는데 각각은 목표가 다르고 공격자의 지식 수준에
# 대한 가정도 다릅니다. 그러나 보통 가장 중요한 목표는 입력 데이터에 최소한의 작은 변화를 
# 추가하여 이것이 의도하는대로 잘못 분류되게 하는 것입니다. 공격자의 지식 수준에 대한 
...
# 이안 갓펠로우가 기고하였습니다.
# 이 공격법은 놀랍도록 강력하지만 직관적입니다. 학습 방식, *그라디언트*를 활용하여 신경망을 공격하도록 
# 설계 되었습니다. 아이디어는 간단합니다. 역전파 그라디언트를 기반으로 가중치를 조정하여 손실을 최소화하기보다는
# 공격이 동일한 역전파 그라디언트를 기반으로 *손실을 최대화하하는 방향으로 입력 데이터를 조정*합니다. 
# 다시 말해 공격은 입력 데이터에서 계산된 손실 그라디언트를 사용하고 입력 데이터를 조정하여 손실이 최대가 되게 합니다.
...
# 공격은 :math:`\nabla_{x} J(\mathbf{\theta}, \mathbf{x}, y)` 계산을 위해 입력 데이터에 그라디언트를 역전파합니다.
# 그러고 나서, 그라디언트는 손실 값이 최대화되는 방향으로 (예를 들면, :math:`sign(\nabla_{x} J(\mathbf{\theta}, \mathbf{x}, y))` ) 
...
# 마지막은 *data_grad* 로 입력 영상 (:math:`\nabla_{x} J(\mathbf{\theta}, \mathbf{x}, y)`) 에 대한 그라디언트 손실 값입니다. 
...
# 정확도을 보고하기 때문입니다. 더 구체적으로 보면 테스트 셋의 각각의 샘플에서 테스트 함수는 입력 데이터에 대한 손실 그라디언트 (:math:`data\_grad`) 를 계산하고, 
...
def test( model, device, test_loader, epsilon ):
    ...
    for data, target in test_loader:
        ...
        # 모델의 그라디언트들을 전부 0으로 설정합니다
        model.zero_grad()

        # 후방 전달을 통해 모델의 그라디언트를 계산합니다
        loss.backward()
