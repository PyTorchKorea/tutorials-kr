...
많은 시퀀스-투-시퀀스 문제들에서 성능이 우월하다는 것이 증명되어 왔습니다.
...
``nn.Transformer`` 모듈은 현재 고도로 모듈화되어 있어,
...
######################################################################
# 에포크 내에서 반복됩니다. 만약 검증 오차 (validation loss) 가 우리가 지금까지 관찰한 것 중 최고라면 모델을 저장합니다.
...
# -------------------------------------
#
# 평가 데이터셋에 대한 결과를 확안하기 위해서 최고의 모델을 적용합니다.
