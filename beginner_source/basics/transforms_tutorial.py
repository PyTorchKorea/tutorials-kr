...
**변환(transform)** 을 해서 데이터를 조작하고 학습에 적합하게 만듭니다.

모든 TorchVision 데이터셋들은 변환 로직을 갖는, 호출 가능한 객체(callable)를 받는 매개변수 두개
...
주로 사용하는 몇가지 변환(transform)을 제공합니다.
...
정답(label)이 필요합니다. 이러한 변환(transformation)을 하기 위해 ``ToTensor`` 와 ``Lambda`` 를 사용합니다.
"""
...
# ------------------------------------------------------------------------------------------
#
# Lambda 변환은 사용자 정의 람다(lambda) 함수를 적용합니다. 여기에서는 정수를 원-핫으로 부호화된 텐서로 바꾸는
# 함수를 정의합니다.
