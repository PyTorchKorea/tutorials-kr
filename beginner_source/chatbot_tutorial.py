...
# 사용할 데이터는 영화 대본으로 구성된 `Cornell Movie-Dialogs(코넬
...
# 모델(Neural Conversational Model) <https://arxiv.org/abs/1506.05869>`__은
...
#    <https://www.cs.cornell.edu/~cristian/Cornell_Movie-Dialogs_Corpus.html>`__을
#    읽어오고 전처리한다
# -  `Luong의 attention 메커니즘 <https://arxiv.org/abs/1508.04025>`__을
#    이용하여 sequence-to-sequence 모델을 구현한다
# -  미니배치를 이용하여 인코더와 디코더를 함께 학습한다
# -  탐욕적 탐색 기법(greedy-search)을 사용하는 디코더 모듈을 구현한다
# -  학습한 챗봇과 대화를 나눈다
#
# **감사의 글**
#
...
# ---------
#
# 시작에 앞서, `여기 <https://www.cs.cornell.edu/~cristian/Cornell_Movie-Dialogs_Corpus.html>`__에서
...
#
# `코넬 대학교의 영화 속 대화 말뭉치 데이터셋
# <https://www.cs.cornell.edu/~cristian/Cornell_Movie-Dialogs_Corpus.html>`__은
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# 편의를 위해 데이터를 형식이 잘 갖춰진 형태로 만들려고 합니다. 각 줄에
# *질의 문장*과 *응답 문장*의 쌍이 탭으로 구분되어 있게끔 하는 것입니다.
#
# 다음의 함수는 *movie_lines.txt* 원본 데이터 파일을 파싱할 수 있도록
# 해줍니다.
#
# -  ``loadLines``는 파일의 각 줄을 변환하여 필드(줄 번호 lineID, 인물
#    번호 characterID, 영화 번호 movieID, 인물 character, 대사 text)에
#    대한 사전으로 만듭니다
# -  ``loadConversations``는 ``loadLines``를 통해 읽어들인 lines의
#    필드를 *movie_conversations.txt*에 나와 있는 내용에 맞춰 대화
#    형태로 묶습니다
# -  ``extractSentencePairs``는 대화(conversations)에서 문장 쌍을
#    추출합니다
#

# 파일의 각 줄을 필드에 대한 사전 형태로 변환합니다
...
# lines의 필드를 *movie_conversations.txt*에 따라 대화 형태로 묶습니다
...
def extractSentencePairs(conversations):
    ...
    for conversation in conversations:
        # 대화를 이루는 각 줄에 대해 반복문을 수행합니다
        for i in range(len(conversation["lines"]) - 1):  # 마지막 줄은 무시합니다(그에 대한 반응이 없으므로)
...
######################################################################
# 이제 이 함수를 호출하여 파일 *formatted_movie_lines.txt*를 만듭니다.
#
...
# 사전 lines, 리스트 conversations, 그리고 각 필드명을 초기화합니다
lines = {}
conversations = []
MOVIE_LINES_FIELDS = ["lineID", "characterID", "movieID", "character", "text"]
MOVIE_CONVERSATIONS_FIELDS = ["character1ID", "character2ID", "movieID", "utteranceIDs"]

# lines를 읽어들여 conversations를 만듭니다
print("\nProcessing corpus...")
...
# ~~~~~~~~~~~~~~~~~~
#
# 다음에 해야 할 일은 단어 사전을 만들고, 질의/응답 문장 쌍을 메모리로
# 읽어들이는 것입니다.
#
# 우리가 **단어**로 구성된 시퀀스를 다룬다는 점에, 그래서 이를 이산한
# 수치 값으로 매핑하는 자연스러운 방법이 따로 없다는 점에 유의하기
# 바랍니다. 따라서 우리는 데이터셋 안에 들어 있는 단어를 인덱스 값으로
# 변환하는 매핑을 따로 만들어야 합니다.
#
# 이를 위해 우리는 ``Voc`` 라는 클래스를 만들어 단어에서 인덱스로의
# 매핑, 인덱스에서 단어로의 역 매핑, 각 단어의 등장 횟수, 전체 단어 수
# 등을 관리하려 합니다. 이 클래스는 단어 사전에 새로운 단어를 추가하는
# 메서드(``addWord``), 문장에 등장하는 모든 단어를 추가하는
# 메서드(``addSentence``), 그리고 자주 등장하지 않는 단어를 쳐내는
# 메서드(``trim``)를 제공합니다. 단어를 쳐내는 내용에 대해서는 뒤에서
# 좀 더 자세히 살펴보겠습니다.
#
...
######################################################################
# 이제 단어 사전과 질의/응답 문장 쌍을 재구성하려 합니다. 그러한 데이터를
...
# 단어 사전에서 제거하는 것입니다. 피처 공간의 크기를 줄이면 모델이
...
# ---------------------------
#
# 상당한 노력을 기울여 데이터를 전처리하고, 잘 정리하여 단어 사전 객체와
# 문장 쌍의 리스트 형태로 만들어두긴 했지만, 결국 우리가 만들 모델에서
# 사용하는 입력은 수치 값으로 이루어진 torch 텐서입니다. 처리한 데이터를
# 모델에 맞는 형태로 준비하는 방법의 하나가 `seq2seq 변환 튜토리얼
# <https://pytorch.org/tutorials/intermediate/seq2seq_translation_tutorial.html>`__에
# 나와 있습니다. 이 튜토리얼에서는 배치 크기로 1을 사용하며, 이는 즉
# 문장에 등장하는 단어를 사전에서의 인덱스로 변환하여 모델에 제공하기만
# 하면 된다는 의미입니다.
#
...
# 것입니다.
#
# `Sutskever 등 <https://arxiv.org/abs/1409.3215>`__은 두 개의 독립된
...
# Recurrent Unit)이 있습니다. 이는 `Cho 등 <https://arxiv.org/pdf/1406.1078v3.pdf>`__이
# 2014년에 고안한 것입니다. 우리는 GRU를 양방향으로 변환한 형태를
...
# 인코더의 문백 벡터를 사용하며, 자체적은 은닉 상태에 따라 시퀀스의 다음
# 단어를 생성하게 됩니다. 디코더는 *EOS_token*, 즉 문장의 끝을 나타내는
# 토큰을 출력할 때까지 계속 단어를 생성합니다. 바닐라 seq2seq 디코더에는
# 잘 알려진 문제점이 있습니다. 만약 우리가 입력 시퀀스의 의미를 인코딩할
...
# <https://arxiv.org/abs/1409.0473>`__은 'attention mechanism'을
# 고안했습니다. 이는 디코더가 매 단계에 대해 고정된 문맥을 계속 사용하는
# 것이 아니라, 입력 시퀀스의 특정 부분에 집중하게 하는 방식입니다.
#
# 높은 차원에서 이야기 하자면, attention은 디코더의 현재 은닉 상태와
# 인코더의 출력을 바탕으로 계산됩니다. 출력되는 attention 가중치는
# 입력 시퀀스와 동일한 모양을 가집니다. 따라서 이를 인코더의 출력과 곱할
# 수 있고, 그 결과로 얻게 되는 가중치 합은 인코더의 출력에서 어느 부분에
# 집중해야 할지를 알려줍니다. `Sean Robertson <https://github.com/spro>`__의
# 그림에 이러한 내용이 잘 설명되어 있습니다.
#
# .. figure:: /_static/img/chatbot/attn2.png
#    :align: center
#    :alt: attn2
#
# `Luong 등 <https://arxiv.org/abs/1508.04025>`__은 Bahdanau의 기초 연구를
# 더욱 발전시킨 '전역(global) attention'을 제안했습니다. '전역 attention'의
# 핵심적인 차이점은 인코더의 은닉 상태를 모두 고려한다는 점입니다. 이는
# Bahdanau 등의 '지역(local) attention)' 방식이 현재 시점에 대한 인코더의
# 은닉 상태만을 고려한다는 점과 다른 부분입니다. '전역 attention'의 또 다른
# 차이점은 attention에 대한 가중치, 혹은 에너지를 계산할 때 현재 시점에 대한
# 디코더의 은닉 상태만을 사용한다는 점입니다. Bahdanau 등은 attention을
# 계산할 때 디코더의 이전 단계 상태에 대한 정보를 활용합니다. 또한 Luong 등의
# 방법에서는 인코더의 출력과 디코더의 출력에 대한 attention 에너지를 계산하는
...
# 모든 상태를 뜻합니다.
#
# 종합해 보면, 전역 attention 메커니즘을 다음 그림과 같이 요약할 수 있을
# 것입니다. 우리가 'attention 레이어'를 ``Attn``라는 독립적인 ``nn.Module``로
...
######################################################################
# 이처럼 attention 서브모듈을 정의하고 나면 실제 디코더 모델을 구현할 수
...
#    3) (2)에서 구한 현재의 GRU 출력을 바탕으로 attention 가중치를 계산합니다.
#    4) 인코더 출력에 attention을 곱하여 새로운 "가중치 합" 문백 벡터를 구합니다.
...
class LuongAttnDecoderRNN(nn.Module):
    ...
    def forward(self, input_step, last_hidden, encoder_outputs):
        ...
        # 현재의 GRU 출력을 바탕으로 attention 가중치를 계산합니다
        attn_weights = self.attn(rnn_output, encoder_outputs)
        # 인코더 출력에 attention을 곱하여 새로운 "가중치 합" 문백 벡터를 구합니다
...
# 나와 있습니다.
#
# 우리는 수렴을 잘 하도록 몇 가지 영리한 전략을 적용합니다.
#
# -  첫 번째 전략은 **teacher forcing**을 사용하는 것입니다. 이는 일정한 확률
#    ``teacher_forcing_ratio``에 따라 현재의 목표 단어를 디코더의 다음 입력인
#    것처럼 활용한다는 의미입니다(디코더가 이번 단계에 예측한 결과 대신에).
#    이 기법은 디코더의 보조 바퀴처럼 작용하여 효율적으로 학습될 수 있게
#    도와 줍니다. 하지만 teacher forcing 기법은 추론 과정에서 모델이 불안정
#    해지도록 할 수도 있는데, 이는 디코더가 학습 과정에서 자신의 출력
...
#    그라디언트를 클리핑 한다는, 또는 최댓값을 넘어간 경우는 최댓값과 동일한
#    값으로 두는 것입니다. 그렇게 함으로써 그라디언트가 지수 함수적으로
#    증가하거나 오버플로를 일으키는(NaN) 경우를 막고, 비용 함수의 급격한
#    경사를 피하겠다는 것입니다.
#
...
# 계속 채팅할 수 있습니다.
#
# 마지막으로, 만약 사전에 포함되어 있지 않은 단어를 포함하고 있는 문장이
