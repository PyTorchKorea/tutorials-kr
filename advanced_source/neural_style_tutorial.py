...
그리는 방식을 이용해 그린 것처럼 결과를 내는 알고리즘입니다:
...
자, 더 나아가려면 수학이 필요합니다. :math:`C_{nn}` 를 미리 학습된 깊은 합성곱 신경망 
네트워크(pre-trained deep convolutional neural network)라고 하고, :math:`X` 를 어떤 이미지라고 해보겠습니다.
:math:`C_{nn}(X)` 은 입력 이미지 X를 입력으로 해서 CNN 네트워크를 통과한 것을 의미합니다.
(모든 레이어들의 특징 맵(feature map)을 포함하는)
...
# CUDA를 쓰기 위해서,  ``torch.cuda.is_available()`` 를 제공하는데, 
...
# ~~~~~~~~~~~~
#
# 다시, 코드를 간단하게 하기 위해, 콘텐츠와 스타일 이미지들의 같은 차원의 것을 가져옵니다.
...
# ~~~~~~~~~~~~~~~~
#
# 알고리즘의 저자인 Len Gatys 가 `여기서 <https://discuss.pytorch.org/t/pytorch-tutorial-for-neural-transfert-of-artistic-style/336/20?u=alexis-jacq>`__, 제안한 바에 따라
# 우리는 경사 하강법을 실행하는데 L-BFGS 알고리즘을 사용 하겠습니다.
...
# 우리는 ``.requires_grad_()`` 를 사용하여 해당 이미지가 그레디언트가 필요함을 확실하게 합니다.
#
