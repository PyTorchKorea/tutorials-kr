...
class ContentLoss(nn.Module):
    ...
    def __init__(self, target,):
        ...
        # 기울기를 동적으로 계산하는데 사용되는 tree로부터 타겟 Content를 `분리(detach)` 합니다.
...
# gram 행렬은 주어진 행렬에 전치 행렬을 곱한 결과 입니다.
# 이 어플리케이션에서 주어진 행렬은 계층 :math:`L` 의 특징 맵 :math:`F_{XL}` 의 재구성 된 버전입니다.
# :math:`F_{XL}` 는 :math:`K`\ x\ :math:`N` 행렬인 :math:`\hat{F}_{XL}` 을 형성하도록 재구성 되며,
# :math:`K` 는 계층 :math:`L` 의 특징 맵의 수이고 :math:`N`는 벡터화 된 특징 맵의 길이입니다.
# 예를 들어, :math:`\hat{F}_{XL}` 의 첫번째 줄은 벡터화 된 특징 맵 :math:`F_{XL}^1` 에 해당합니다.
#
# 마지막으로, gram 행렬은 행렬에서 각 값을 값의 총 개수로 나누어 정규화 해야합니다.
...
# 이미지로 훈련됩니다.
# 이미지를 네트워크로 입력하기 전에 정규화 하는데 사용합니다.
#
...
class Normalization(nn.Module):
    def __init__(self, mean, std):
        ...
        # B는 배치 크기입니다. C는 채널의 수 입니다. H는 높이고 W는 너비입니다.
...
######################################################################
# 마지막으로 뉴럴 변환(neural transfer)을 수행하는 함수를 정의해야합니다.
# 네트워크의 각 반복 동안, 업데이트 된 입력이 주어지고 새로운 손실을 계산합니다.
