...
이 튜토리얼은 모델의 정확성을 더욱 높이기 위한 두 가지 발전된 기술 - 채널별 양자화(per-channel quantization)와 양자화 자각 훈련(quantization-aware training)- 을 설명할 뿐 아니라 어떻게 훈련 후 정적 양자화(post-training static quantization)가 동작하는지 보여줍니다. 
...
게다가 `여기서 <https://arxiv.org/abs/1806.08342>`_ 보인 발전된 양자화 기술들을 쉽게 적용시키는 법을 볼 수 있을 것이며 이와 같은 고급 양자화 기술들이 일반적인 양자화 기법보다 정확도에 훨씬 적은 타격을 미친다는 것을 확인하실 수 있습니다.

주의: ``MobileNetV2`` 모델 구조 정의하기, 데이터 로더 정의하기 등과 같은 다른 PyTorch 저장소에서 공통으로 사용되는 많은 코드를 사용할 예정입니다. 
물론 처음부터 읽으면 좋겠지만 양자화 특징에 대해 알고 싶다면 "4. 훈련 후 정적 양자화" 부분으로 넘어가면 됩니다. 
...
# - ReLU6를 ReLU로 교체하기
#
# 알림: 이 코드는 `이곳에서 <https://github.com/pytorch/vision/blob/master/torchvision/models/mobilenet.py>`_ 가져왔습니다. 
...
# ^^^^^^^^^^^^^
#
# 이 튜토리얼에서 만드는 데이터셋은 ImageNet 데이터 안의 1000개 클래스에 대해, 클래스별로 각각 1장의 이미지를 포함하고 있습니다. 
...
# 
#
# 반면에, 전체 ImageNet 데이터셋을 사용해 이 튜토리얼에 있는 코드를 실행하기 위해서는 `여기서 <https://pytorch.org/docs/stable/torchvision/datasets.html#imagenet>`_ ``torchvision`` 을 사용해 데이터를 다운받을 수 있습니다.
...
######################################################################
# 이미지 300개에서 78%의 정확도를 볼 수 있는데, 이는 ImageNet의 확실한 기준이며, 특히 우리의 모델이 14.0MB에 불과하다는 점을 고려할 때 더욱 그렇습니다. 
#
# 이 모델이 비교할 기준이 됩니다. 다음으로, 다른 양자화 방법을 사용해봅시다. 
#
# 4. 훈련 후 정적 양자화(Post-training static quantization)
# ---------------------------------------------------------
#
# 훈련 후 정적 양자화는 동적 양자화처럼 가중치를 실수에서 정수로 변환하는 것을 포함할 뿐 아니라 추가적인 첫 번째 피딩(feeding) 네트워크를 지나가는 데이터의 배치 단계의 수행과 다른 활성값의 분산 결과 계산이 필요합니다
...
# 특히, 이 추가적인 단계는 양자화된 값이 모든 연산 사이에서 실수로 변환하는 것 대신 - 그리고 다시 정수로 돌아감 - 이 값들이 연산들을 지나갈 수 있도록 만들어 상당한 속도 향상을 이룹니다. 
...
######################################################################
# 양자화된 모델에 대해, 같은 300개 이미지에 대해서 62% 이하인 상당히 낮은 정확성을 보입니다.
# 그럼에도 불구하고, 모델의 사이즈를 거의 4배 정도가 줄은 3.6 MB 이하로 감소시켰습니다.
#
# 추가적으로, 단순히 다른 양자화 형태를 사용하면 정확도을 꽤 향상시킬 수 있습니다. 
# x86의 양자화를 위해 추천하는 형태를 통해 같은 행위를 반복하면 됩니다. 
# 이 형태는 다음과 같습니다. 
#
# - 각 채널 기본에 있는 가중치를 양자화합니다.
# - 활성값의 히스토그램을 모으는 히스그램 옵저버(observer)를 사용해 최선의 방법인 양자화 매개변수를 고릅니다.
#
...
######################################################################
# 단지 이 양자화 형태 메소드를 바꾸는 것만으로 76%나 정확성이 증가합니다!
...
######################################################################
# 높은 정확성을 가진 양자화된 모델을 훈련하기는 추론할 시에 정확한 숫자 모델링을 요구합니다. 
# 그러므로 양자화 자각 훈련을 위해, 다음을 통해 훈련을 수정합니다:
#
# - 추론 수치와 더 많은 일치를 위해 훈련의 후반부에는 추론 시와 동일하게 배치 정규화에 쓰이는 평균과 분산 값을 이동 평균으로써 산출합니다. 
