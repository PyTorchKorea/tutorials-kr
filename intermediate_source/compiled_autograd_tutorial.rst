...
.. grid:: 2
    ...
    .. grid-item-card:: :octicon:`mortar-board;1em;` 배울수 있는 내용
...
개요
--------
Compiled Autograd는 PyTorch 2.4 에서 소개된  ``torch.compile`` 의 확장 기능으로, 더 큰 backward 그래프를 캡쳐할 수 있게 해줍니다.

``torch.compile`` 이 backward 그래프를 포착하긴 하지만, 그것은 **부분적으로만** 이루어집니다. AOTAutograd 컴포넌트는 backward 그래프를 사전에 캡쳐하지만, 몇 가지 제한사항이 존재합니다.
...
* 더 큰 캡쳐 때문에 dynamo 에서 재컴파일과 그래프 끊김이 발생하기 쉽습니다.
...
위에 있는 코드는 ``Model`` 클래스 인스턴스를 만들고 ``torch.randn(10)`` 을 사용하여 무작위 10차원의 tensor ``x`` 를 만듭니다.
훈련 루프 함수 ``train`` 을 정의하고, 실행 최적화를 위해 ``@torch.compile`` 로 지정합니다.
...
* ``AOTDispatcher`` 는 훅을 비활성화하고 autograd 엔진을 호출하여 ``model.linear.weight`` 와 ``model.linear.bias`` 의 변화도를 계산하며 연산을 그래프로 기록합니다. ``torch.autograd.Function`` 을 사용하여, AOTDispatcher는 ``train`` 함수의 forward와 backward 구현을 재작성합니다.
...
* 더 많은 tensor 메타데이터와 재컴파일 이유까지 출력하고 싶다면, 성능이 저하되는 대신 ``TORCH_LOGS="compiled_autograd_verbose" python example.py`` 를 사용하세요

위의 스니펫을 다시 실행하면, compiled autograd 그래프가 ``stderr`` 에 로깅이 됩니다.
...
-------------------------------------------------------------
두 가지의 컴파일에 대해 서로 다른 컴파일러 설정을 사용할 수 있습니다, 예를 들어 forward에 그래프 분절이 있더라도 backward는 full graph로 설정할 수 있습니다.
...
또는 context manager를 사용할 수 있으며, 해당 스코프 안의 모든 autograd 호출에 적용될 것입니다.
...
위의 예제에서, ``x`` 의 형태가 변경되면, compiled autograd는 첫 번째 변경 이후 ``x`` 를 동적 형태의 tensor로 표시합니다. 이로 인해 재컴파일 메시지가 (**Cache miss due to changed shapes**) 나타나는 것을 확인할 수 있습니다.
