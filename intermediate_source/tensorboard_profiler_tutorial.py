...
GPU측에서 실행되는 CUDA 커널 기록을 수행할 수 있는 업데이트된 프로파일러 API를 포함하고 있다.

프로파일러는 텐서보드 플러그인에 이 정보를 시각화 해줄수 있고 성능 병목들의 분석을 제공해줄 수 있다.

이 튜토리얼에서는, 모델 성능 분석을 위한 텐서보드 플러그인을 어떻게 사용하지는 설명하기 위해서
간단한 Resnet 모델을 사용할 것이다.
Setup
설정
-----
``torch``와``torchvision``을 설치하기 위해서 다음 커맨드를 사용한다:
::
...
#
# 1. 데이터와 모델을 준비한다.
# 2. 프로파일러를 사용하여 실행 이벤트를 기록한다.
# 3. 프로파일러를 실행한다.
# 4. 결과를 확인하고 모델 성능 분석을 위해 텐서보드를 사용한다.
# 5. 프로파일러의 도움으로 성능을 증진시킨다.
# 6. 다른 고급 기능으로 성능을 분석한다.
#
# 1. 데이터와 모델을 준비한다.
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# 첫번째, 모든 필요한 라이브러리를 import 한다:
#
...
######################################################################
# 그후 입력데이터를 준비한다. 이 튜토리얼에서는 CIFAR10 데이터셋을 사용한다.
# 원하는 포맷으로 데이터를 변환하고 데이터로더를 사용하여 각각 배치크기로 로드한다.
...
######################################################################
# 다음으로, Resnet 모델,손실함수,최적화 객체들을 만든다.
# GPU 실행을 위해서, 모델과 로스를 GPU 장치로 이동시킨다.
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# 프로파일러는 컨텍스트 매니저를 통해 활성화 되고 가장 유용한 여러 파라미터를을 수용한다:
#
# - ``schedule`` - 호출가능한 스케줄은 단일 파라미터로 단계를 수행하고
#   각 단계로 수행할 프로파일러 작업을 반환합니다.
#   이 예제에서는 다음과 같다. ``wait=1, warmup=1, active=3, repeat=2``,
#   프로파일러는 첫 번째 단계를 건너뛰고,
#   두 번째에서 워밍업을 시작하고,
#   세개의 반복 기록한다,
#   그 후 트레이스를 사용할 수 있게되고 on_profiler_ready가 호출된다.
#   종합적으로,이 싸이클이 두번 반복된다. 각 싸이클은 텐서보드 플러그인에서 "span"이라고 불려진다.
#
#   ``wait`` 단계 동안, 프로파일러는 비활성화 된다.
#   ``warmup``단계 동안, 프로파일러가 추적을 시작하지만 결과는 무시된다.
#   프로파일링 오버헤드를 줄이기 위한 것이다.
#   프로파일링 시작 시 발생하는 오버헤드는 크고 프로파일링 결과를 왜곡하기 쉽다..
#   ``active`` 단계 동안, 프로파일러는 이벤트를 처리하고 기록한다.
# - ``on_trace_ready`` - on_trace_ready는 각 단계의 끝에 호출 된다.
#   이 예에서는 텐서보드에 대한 결과 파일을 생성하기 위해 ``torch.profiler.tensorboard_trace_handler``를 사용한다.
#   프로파일링 후 결과 파일은 ``.log/resnet18`` 디렉토리에 저장된다.
#   TensorBoard에서 프로파일을 분석하기 위해 이 디렉토리를 ``logdir`` 매개변수로 지정한다.
# - ``record_shapes`` - 연산자 입력의 모양을 기록할지 여부를 나타낸다.
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# 위의 코드를 실행합니다. 프로파일링 결과는 "..log/resnet18" 디렉토리에 저장된다.
...
# ``profile_memory=True`` 를 활성화하는 데는 약 몇분의 시간이 소요 될 것 이다.
...
#
# "Size Increase" 모든 할당 바이트를 더하고 모든 메모리 릴리즈 바이트를 뺀다.
#
# The "Allocation Size"는 메모리 릴리스를 고려하지 않고 모든 할당 바이트를 더한다.
#
# - Distributed view
