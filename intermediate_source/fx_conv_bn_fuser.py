...
1) 데이터 의존성에서 합성곱/배치 정규화(conv/batch norm) 패턴을 찾습니다.
2) 1번에서 발견한 패턴의 경우 배치 정규화 통계(batch norm statistics)를 합성곱 가중치(convolution wegiths)로 결합(folding)합니다.

이 최적화는 추론 모드(즉, `mode.eval()`)의 모델에만 적용된다는 점에 유의하세요.

다음 링크에 있는 결합기(fuser)를 만들어보겠습니다:
...
######################################################################
#
# 먼저, 몇 가지의 import 과정을 먼저 처리해줍시다(나중에 코드에서 모두 사용할 것입니다).
...
# ``BatchNorms`` 중 하나는 또 다른 Module로 감싸져 있습니다.
...
# PyTorch가 연산 그래프에 쉽게 접근할 수 있는 방법을 제공하지 않는다는 것입니다.
...
####################################
# FX 결합(FX Fusion) 전달(pass)
# --------------------------------
# 이제 합성곱과 배치 정규화를 결합하는 방법뿐만 아니라 연산 그래프도 얻었으므로
