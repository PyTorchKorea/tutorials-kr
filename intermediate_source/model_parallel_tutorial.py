...
이 방법은 신경망 모델을 상당히 빠르게 학습시킬 수 있는 장점이 있지만, 신경망 모델이 하나의 GPU에 할당될 수 있도록 모델 크기가 제한되는 단점이 있습니다.

이번 튜토리얼에서는 ''데이터 병렬 처리'' 가 아닌 **모델 병렬 처리** 문제를 해결하는 방법을 소개합니다.
각 GPU에 모델 전체를 복제하는 것이 아닌, 하나의 모델을 여러 GPU에 분할하여 할당하는 방법입니다.
구체적으로, 10개의 층으로 구성된 ''m'' 신경망 모델에 대해서 ''데이터 병렬 처리'' 방법은 10개의 층을 전부 복제하여 각 GPU에 할당한다면, 
...
######################################################################
# 위의 예제는 각 층, 혹은 텐서값을 GPU에 할당하는 것 외에는 단일 GPU로 신경망 모델을 구현하는 것과 매우 유사한 구조인 것임을 확인할 수 있습니다.
# 다시 말해, GPU에 텐서값 혹은 층을 할당하는 것 외에는 추가적으로 설정하는 부분이 없습니다.
# 역전파 계산하는 부분과, 최적화하는 부분은 단일 GPU를 이용하여 신경망 모델의 가중치값을 업데이트하는 것 처럼, 자동으로 오차에 의한 기울기값을 반영합니다.
