...
# 일정량의 데이터를 수집하고 즉시 사용해 정책(policy)이 근접성 제약(proximality constraint)를 적용하면서 
...
# 이를 통해 훈련 중에 즉석에서 학습되는 어떤 유틸리티 추정치에 의존함으로써
...
# 훈련 루프에 데이터를 전달하기 위해 :class:`~tensordict.TensorDict`를 사용하면, 롤아웃 내용의
...
# 서브 배치를 샘플링할 수도 있지만, 이 클래스들을 사용하면 내부 훈련 루프를
...
# 훈련 루프
# -------------
# 이제 우리는 훈련 루프를 코딩하는 데 필요한 모든 요소를 갖추었습니다.
# 단계는 다음과 같습니다.
#
# * 데이터 수집
#
