...
* we can use DTensor and DCP APIs to manipulate parameters to get full state dict, see "state dict" section below for details. For distributed state dicts, we can save/load checkpoints (`doc <https://docs.tutorials.pytorch.kr/recipes/distributed_checkpoint_recipe.html>`_) without extra communication
