...
(실험버전) PyTorch를 사용한 Channels Last Memory Format
*******************************************************
**Author**: `Vitaly Fedyunin <https://github.com/VitalyFedyunin>`_
**번역**: `최윤정 <https://github.com/potatochips178>`_
...
예를 들어, 전형적인(연속적인) NCHW tensor의 저장소(이번 경우는 2개의 2x2 이미지에 3개의 채널이 존재함) 는 다음과 같이 보입니다.
...
Pytorch는 존재하는 스트라이드(strides) 구조를 사용함으로써 메모리 형식을 지원합니다(그리고  eager, JIT, 그리고 TorchScrip를 포함한 존재하는 모델들과 하위 호환성을 제공합니다). 
...
print(x.shape) # Outputs: (10, 3, 32, 32) as dimensions order preserved # 차원 순서는 보존하면서
...
# ``empty_like``, ``*_like`` 연산자는 메모리 형식능 보존합니다. 
...
if torch.backends.cudnn.version() >= 7603:
    ...
    model = model.to(memory_format=torch.channels_last) # Module parameters need to be Channels Last # 모듈 인자들은 Channels Last로 변환이 필요합니다 
...
# 가장 중요한 성능 향상은 Tensor Cores 지원과 함께하는 NVidia의 하드웨어에거 관찰됩니다. 
# NVidia가 지원하는 스크립트를 훈련하는 AMP(Automated Mixed Precision)를 실행시키는 동안 22% 넘는 성능향상 이룰 수 있습니다. 
...
# Channels Last 지원은 기존 모델에 의해 제한되지 않고 어떠한 모델도 Channels Last로 바뀔 수 있으며 입력이 정확히 형식에 맞춰지면 바로 그래프를 통해 형식을 전할 수 있습니다. 
#
...
# 그러나, 모든 연산자들이 Channels Last를 지원하도록 완전히 바뀐것은 아니다.
# (보통 대신에 연속적인 출력을 반환합니다)
...
# 아래 코드를 실행시킨 후, 만약 연산자들의 출력이 입력의 메모리 형식과 일치하지 않는다면 연산자들은 예외를 일으킬것입니다. 
#
#
