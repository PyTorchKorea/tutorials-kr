...
(DDP)은 PyTorch에서 모델을 여러 머신에 걸쳐 병렬화할 수 있게 해주는 강력한 모듈로,
대규모 딥러닝 애플리케이션에 적합합니다.
DDP를 사용하려면 여러 프로세스를 생성하고 각 프로세스마다 하나의 DDP 인스턴스를 만들어야 합니다.

DDP는 어떻게 동작할까요? DDP는
`torch.distributed <https://tutorials.pytorch.kr/intermediate/dist_tuto.html>`__
패키지의 집합 통신(collective communications)을 사용하여 모든 프로세스 간에 변화도(gradient)와 버퍼를 동기화합니다.
즉, 각 프로세스가 모델의 자체 복사본을 갖지만, 마치 단일 머신에서 학습하는 것처럼 모든 프로세스가 함께 협력하여 모델을 학습합니다.

이를 위해 DDP는 모델의 각 매개변수에 autograd 훅(hook)을 등록합니다.
역전파 전달이 실행되면 이 훅이 발동되어 모든 프로세스 간에 변화도 동기화를 트리거합니다.
이를 통해 각 프로세스가 동일한 변화도를 갖게 되며, 이 변화도가 모델 업데이트에 사용됩니다.

DDP의 동작 원리와 효과적인 사용법에 대한 자세한 정보는
`DDP 설계 문서 <https://pytorch.org/docs/master/notes/ddp.html>`__ 를 참고하세요.
DDP를 사용하면 이전보다 더 빠르고 효율적으로 모델을 학습할 수 있습니다!

DDP를 사용하는 권장 방법은 각 모델 복제본(replica)마다 하나의 프로세스를 생성하는 것입니다.
모델 복제본은 여러 장치에 걸쳐 배치될 수 있습니다.
DDP 프로세스는 동일한 머신 또는 여러 머신에 걸쳐 배치할 수 있습니다.
GPU 장치는 DDP 프로세스 간에 공유할 수 없습니다 (즉, 하나의 GPU에 하나의 DDP 프로세스).


이 튜토리얼에서는 기본적인 DDP 사용 사례부터 시작하여 체크포인트 저장 및
모델 병렬 처리와 DDP를 결합하는 고급 사용 사례까지 다룹니다.
...
.. code:: python
    ...
    def setup(rank, world_size):
        ...
        # `가속기 <https://pytorch.org/docs/stable/torch.html#accelerators>`__ 에서
        # CUDA, MPS, MTIA, XPU 등의 모델을 학습할 수 있습니다.
...
torch.distributed.run/torchrun으로 DDP 초기화하기
---------------------------------------------------

PyTorch Elastic을 활용하면 DDP 코드를 간소화하고 작업을 더 쉽게 초기화할 수 있습니다.
동일한 Toymodel 예제를 사용하여 ``elastic_ddp.py`` 라는 파일을 만들어 보겠습니다.
...
위 예제에서는 DDP 스크립트를 두 개의 호스트에서 실행하며, 각 호스트마다 8개의 프로세스를 실행합니다.
즉, 총 16개의 GPU에서 이 작업을 실행하게 됩니다. ``$MASTER_ADDR`` 은 모든 노드에서 동일해야 합니다.

여기서 ``torchrun`` 은 8개의 프로세스를 시작하고, 실행된 노드의 각 프로세스에서
``elastic_ddp.py`` 를 호출합니다. 다만 실제로 이 명령을 2개의 노드에서 실행하려면
slurm과 같은 클러스터 관리 도구를 사용해야 합니다.

예를 들어, SLURM이 활성화된 클러스터에서는 위 명령을 실행하는 스크립트를 작성하고
``MASTER_ADDR`` 을 다음과 같이 설정할 수 있습니다:
...
이후 SLURM 명령으로 이 스크립트를 실행하면 됩니다: ``srun --nodes=2 ./torchrun_script.sh``.

이것은 하나의 예시일 뿐이며, 원하는 클러스터 스케줄링 도구를 선택하여 ``torchrun`` 작업을 시작할 수 있습니다.

Elastic 실행에 대한 자세한 내용은
`빠른 시작 문서 <https://pytorch.org/docs/stable/elastic/quickstart.html>`__ 를 참고하세요.