...
# 딥러닝 모델 생성
# --------------
# 이번 튜토리얼에서는, 얀 르쿤 교수님의 연구진들이 1998년도에 발표한 의 'LeNet 
# <http://yann.lecun.com/exdb/publis/pdf/lecun-98.pdf>` 모델 구조를 이용합니다.
...
# 여기에는 2개의 파라미터값들인 가중치값과 편향값을이 포함될 것이며, 버퍼는 존재하지 않을 것입니다.
...
# 첫 번째로는, ``torch.nn.utils.prune`` 혹은 ``BasePruningMethod`` 의 서브 클래스로 구성된 
# <extending-torch-nn-utils-pruning-with-custom-pruning-functions> 내 존재하는 가지치기 기법을 선택합니다.
...
######################################################################
# 수정이 되지 않은 상태에서 순전파를 진행하기 위해서는 가중치값 속성이 존재해야 합니다.
...
# 이러한 방식은, 가지치기 기법을을 반복적으로 적용해야 하는 경우 마스크를 결합하는 방법을 결정하기 위해 필요합니다.
# 즉, 이미 가지치기 기법이 적용된 모델에 대해서 가지치기 기법을 적용할 때, 
# 기존의 가지치기 기법이 적용되지 않은 파라미터 값에 대해 가지치기 기법이 영향을 미칠 것으로 예상됩니다.
# ``PRUNING_TYPE``을 지정한다면, 가지치기 기법을 적용하기 위해 파라미터값을 올바르게 제거하는 
# ``PruningContainer`` (마스크 가지치기 기법을 반복적으로 적용하는 것을 처리하는) 을 가능하게 합니다.

# 예를 들어, 다른 모든 항목이 존재하는 텐서를 가지치기 기법을 구현하고 싶을 때,
# (또는, 텐서가 이전에 가지치기 기법에 의해 제거되었거나 남아있는 텐서에 대해)
# 한 층의 개별 연결에 작용하며 전체 유닛/채널 (``'구조적'``), 또는 다른 파라미터 간 
# (``'전역 변수'``) 연결에는 작용하지 않기 때문에 ``PRUNING_TYPE='unstructured'`` 방식으로 진행됩니다.
...
def foobar_unstructured(module, name):
    ...
    다음 내용에 따라 모듈을 수정 (또는 수정된 모듈을 반환):
        ...
        기존의 파라미터는 `name+'_orig'` 이름이라는 새로운 매개변수에 저장됩니다.
