...
#
# \* 이미지 분류기의 예측을 해당 이미지의 특징(features)에 기여(attribution)합니다.
#
# \* 기여(attribution) 결과를 시각화 합니다.
...
normalize = transforms.Compose([
    ...
    transforms.Normalize(                # 0이 중심인 imagenet 픽셀의 rgb 분포를 따르는 정규화
...
# 기여(attribution) 결과를 통합해서 반환하는
# ``attribute(...)`` 메소드를 가지는 ``Attribution`` 의 서브클래스 입니다.
#
# 이미지일때 기여(attribution) 결과를 시각해 보겠습니다.
#
...
# 기여(attribution) 결과를 시각화 할 수 있는 즉시 사용가능한 방법을 제공합니다.

import numpy as np
from captum.attr import visualization as viz

# 계산 속성 Tensor를 numpy 배열 이미지로 변환합니다.
attribution_dog = np.transpose(attribution_dog.squeeze().cpu().detach().numpy(), (1,2,0))

vis_types = ["heat_map", "original_image"]
vis_signs = ["all", "all"] # "positive", "negative", 또는 모두 표시하는 "all"
# positive 속성은 해당 영역의 존재가 예측 점수를 증가시킨다는 것을 의미합니다.
# negative 속성은 해당 영역의 존재가 다른 예측 점수를 증가시키는 주의 영역을 나타냅니다.
...
# 입력 텍스트 위에 기여(attribution)를 탐색할 수 있는 보기를 제공합니다.
