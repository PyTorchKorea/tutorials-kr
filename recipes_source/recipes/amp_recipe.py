...
이 레시피를 파이썬 스크립트로 다운로드하고 실행해 볼 수 있습니다. 환경은 Pytorch 1.6+와 쿠다를 지원하는 GPU가 필요합니다.
...
# 간단한 신경망
# ------------------------
# 아래 선형 계층과 ReLU 활성함수(rectified-linear activation function)의 연속으로 이루어진 모델이 혼합 정밀도로 속도 향상이 될 것입니다.
...
# GradScaler 사용하기
# ----------------------------------
# `Gradient scaling <https://pytorch.org/docs/stable/amp.html#gradient-scaling>`_ 은 혼합 정밀도 훈련 시 작은 크기의 그래디언트 값이 0으로 바뀌는 underflowing 현상을 막습니다.
# 
# `torch.cuda.amp.GradScaler <https://pytorch.org/docs/stable/amp.html#torch.cuda.amp.GradScaler>`_ 는 그래디언트를 크기를 편리하게 조절합니다.
...
for epoch in range(0): # 0 epochs, 이 섹션은 설명을 위한 스크립트입니다.
    for input, target in zip(data, targets):
        ...
        # 스케일된 손실값. 스케일된 그래디언트를 만들기 위해, backward()를 스케일된 손실값에 적용해주세요.
        scaler.scale(loss).backward()

        # sacler.step() 은 먼저 옵티마이저에 할당된 파라미터들을 unscale합니다.
        # 이 그래디언트들이 inf나 NaN을 가지고 있지 않다면, optimizer.step()이 호출됩니다.
        # 가지고 있다면, optimizer.step()은 건너뜁니다.
        scaler.step(opt)

        # 다음 루프를 위해 스케일을 업데이트 합니다.
        scaler.update()
...
##########################################################
# 그래디언트를 검사하고 수정하기(그래디언트 클리핑)
# -------------------------------------------------------------------------
# ``scaler.scale(loss).backward()`` 로 생성된 모든 그래디언트는 스케일링됩니다. 
...
for epoch in range(0): # 0 epochs, 이 섹션은 설명을 위한 스크립트입니다.
    for input, target in zip(data, targets):
        ...
        # 그래디언트의 스케일을 원래 스케일로 되돌립니다.
        scaler.unscale_(opt)

        # 옵티마이저에 할당된 파라미터들의 그래디언트가 원래의 스케일로 돌아왔기 때문에, 평소처럼 클리핑을 합니다.
...
# 
# 저장을 할 때는, scaler의 state dict를 모델과 옵티마이저의 state dict과 함께 저장합니다. 
...
##########################################################
# 재시작 할 때는, scaler의 state dict을 모델과 옵티마이저와 함께 가져옵니다.
