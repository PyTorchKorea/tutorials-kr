...
우리는 신경망을 설계할 때 여러 트레이드오프를 마주하게 됩니다. 모델을
개발하고 학습할 때 순환 신경망의 레이어나 패러미터의 개수를 바꿔볼 수
있을 텐데, 그럴 때면 정확도와 모델의 규모나 응답 속도(또는 처리량)
사이에 트레이드오프가 생기게 됩니다. 그러한 변화를 줄 때면 시간과
컴퓨터 자원이 많이 소모되는데, 이는 모델 학습 과정에 대해 반복 작업을
수행하게 되기 때문입니다. 양자화 기법을 기법을 사용하면 이미 알려진
...
형태로 만들어야 한다는 부담이 적은 편입니다. 사실 이러한 작업은 LSTM
모델을 배포용으로 변환할 때 거의 필수적으로 필요한 단계입니다.
...
1. 여러분은 아주 작은 LSTM 네트워크를 가지고 시작할 것입니다
2. 여러분은 단순하게 네트워크를 랜덤한 은닉 상태로 초기화할 것입니다
3. 여러분은 네트워크를 랜덤한 입력으로 테스트할 것입니다.
4. 이 튜토리얼에서는 네트워크를 학습하지 않을 것입니다.
5. 여러분은 시작할 때의 부동소수점 실수를 사용하는 네트워크를 양자화
   했을 때 규모가 줄어들고 수행 속도가 빨라짐을 살펴볼 것입니다.
6. 여러분은 네트워크를 출력값이 FP32 네트워크와 크게 다르지 않음을
   살펴보겠지만, 실제로 학습된 네트워크의 정확도 손실 기댓값이 어떻게
   되는지는 살펴보지 않을 것입니다.
...
class lstm_for_demonstration(nn.Module):
  """기초적인 LSTM모델로, 단순히 nn.LSTM 를 래핑한 것입니다.
...
# 크게 다르지 않다는 점을 살펴보는 것은 의미가 있다고 봅니다.
#
